...
def check_sol_dec_rad(sd):
    ...
    """
    if type(sd) == np.float64:
        if not (sd>=_MINSOLDEC_RADIANS) & (sd <= _MAXSOLDEC_RADIANS):
            raise ValueError(
                'solar declination outside valid range {0!r} to {1!r} rad: {2!r}'
                .format(_MINSOLDEC_RADIANS, _MAXSOLDEC_RADIANS, sd))
    else:
        if not all((sd>=_MINSOLDEC_RADIANS) & (sd <= _MAXSOLDEC_RADIANS)):
            raise ValueError(
                'solar declination outside valid range {0!r} to {1!r} rad: {2!r}'
                .format(_MINSOLDEC_RADIANS, _MAXSOLDEC_RADIANS, sd))
...
def check_sunset_hour_angle_rad(sha):
    ...
    See http://mypages.iit.edu/~maslanka/SolarGeo.pdf
    """
    if type(sha) == np.float64:
        if not (sha>=_MINSHA_RADIANS) & (sha <= _MAXSHA_RADIANS):
            raise ValueError(
                'sunset hour angle outside valid range {0!r} to {1!r} rad: {2!r}'
                .format(_MINSHA_RADIANS, _MAXSHA_RADIANS, sha))
    else:
        if not all((sha>=_MINSHA_RADIANS) & (sha <= _MAXSHA_RADIANS)):
            raise ValueError(
                'sunset hour angle outside valid range {0!r} to {1!r} rad: {2!r}'
                .format(_MINSHA_RADIANS, _MAXSHA_RADIANS, sha))
